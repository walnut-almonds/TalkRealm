...
- **絕不 log 整個 config struct**：`cmd/server/main.go` 曾有 `logger.Info("config", "config", cfg)`，把 JWT secret／DB/Redis 密碼／OAuth client secret／Minio、LiveKit 金鑰／DeepL、LLM API key 全部明文寫進 docker log，release 模式照印（2026-08-18 移除）。新增啟動 log 只能列白名單欄位（version/mode/port），不要整包丟進去。
- **Prod 部署鏈路**：GitHub secrets → `envsubst < configs/config.prod.yaml > config.yaml` →（`.github/workflows/deploy.yml`）scp `config.yaml`/`.env`/`docker-compose.prod.yml`/`nginx.conf` 到 VPS `/opt/talkrealm/` → `docker compose` 起 `ghcr.io/walnut-almonds/talkrealm:latest`。改 GitHub secret **不會**自己生效，必須重跑 deploy 才會重新產生 config.yaml 上傳。
  - workflow 只吃 `push: [main, master]`，沒有 `workflow_dispatch`。不想開空 commit 就 `gh run rerun <run-id>`——secret 是執行當下才讀，rerun 會抓到新值。
  - VPS 上除錯：`ssh myvps`，`docker logs talkrealm-app`；app 掛掉時 nginx 直接回 502（Cloudflare 正常不代表後端活著）。
- **`jwt.secret` 在 release 模式必須 >= 32 字元**（`pkg/config/config.go` 的 `usableJWTSecret`，HS256 的 key 下限 RFC 7518 §3.2），不符就 `log.Fatalf` 拒絕啟動 → crash loop → 502。deploy.yml 已加 CI 守門先擋。產生方式用 `openssl rand -base64 32`（44 字元 / 256-bit 熵）；注意檢查只數字串長度，不等於熵。換 secret = 所有既存 token 失效，全站重新登入。
- **Container log 不輪替會吃掉磁碟（2026-08-18 修復）**：`talkrealm-nginx` 曾累積 21GB（45G 磁碟的 25G）。根因是 port 80 的 HTTP→HTTPS 轉址 block 把每一筆掃描都寫進 log：它是 port 80 的 default server，直連 origin IP 的 bot 全落這裡（最後 20 萬筆裡 99.2% 是兩個 IP 拿 301，只有 49 筆 200）。已在 `nginx/nginx.conf` 該 block 加 `access_log off;`，並在 `docker-compose.prod.yml` 用 `x-logging` anchor 給六個 service 都上 `max-size: 50m` / `max-file: 3` 保底。注意：改 logging 設定會讓 `docker compose up -d` 重建所有容器，舊 container 的 log 檔會跟著被刪（磁碟自動回收）。另 `/opt/talkrealm/{.env,config.yaml}` 權限是 `644`（world-readable，內含全部明文憑證），scp-action 上傳後沒 chmod，機器上目前沒有其他非 root 帳號才沒出事。
