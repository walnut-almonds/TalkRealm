...
func (h *ChannelHandler) CreateChannel(c *gin.Context) {
	...
	if err != nil {
		...
		if err == service.ErrNotGuildMemberCh {
			c.JSON(http.StatusForbidden, gin.H{"error": "not a member of this guild"})
			return
		}
		if err == service.ErrInvalidChannelType {
			c.JSON(http.StatusBadRequest, gin.H{"error": "invalid channel type, must be 'text' or 'voice'"})
			return
		}
