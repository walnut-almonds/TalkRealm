...
func (r *feedPostRepository) TimelineCursor(
	authorIDs []uint,
	before uint,
	limit int,
...
func (r *feedPostRepository) ByAuthorCursor(
...
func (r *feedPostRepository) RecentCandidates(
...
func (r *feedPostRepository) DeleteCascade(postID uint) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.
			Where("comment_id IN (SELECT id FROM feed_comments WHERE post_id = ?)", postID).
			Delete(&model.FeedCommentLike{}).Error; err != nil {
			return err
		}
		...
		if err := tx.Where("post_id = ?", postID).
			Delete(&model.FeedPostAttachment{}).
			Error; err != nil {
			return err
		}
